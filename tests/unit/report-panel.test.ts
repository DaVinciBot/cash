...
vi.mock('$lib/settings/report', async (importOriginal) => {
	const actual = await importOriginal<typeof reportModule>();
	return { ...actual, submitReport: mocks.submitReport };
});

import type * as reportModule from '../../src/lib/settings/report';
import ReportPanel from '../../src/lib/components/settings/ReportPanel.svelte';
